...
async function* run(normalizedOptions, context, extensions) {
    const { codeCoverage, projectSourceRoot, reporters, watch, workspaceRoot, setupFiles, browsers, debug, buildTarget, include, exclude, } = normalizedOptions;
    ...
    const buildTargetOptions = (await context.validateOptions(await context.getTargetOptions(buildTarget), await context.getBuilderNameForTarget(buildTarget)));
    buildTargetOptions.polyfills = (0, options_1.injectTestingPolyfills)(buildTargetOptions.polyfills);
    const outputPath = (0, path_1.toPosixPath)(node_path_1.default.join(workspaceRoot, generateOutputPath()));
    const buildOptions = {
        ...buildTargetOptions,
        watch,
